...
const Sede = () => {
    const [records, setRecords] = useState<ISede[]>([]);
    const [search, setSearch] = useState("");
    const [currentPage, setCurrentPage] = useState(1);
    const [total, setTotal] = useState(0);
    const [isLoading, setIsLoading] = useState(false);
    const [selectedSede, setSelectedSede] = useState<ISede | null>(null);
    ...
    const fetchSedes = async () => {
        try {
            setIsLoading(true);
            const res = await API.getSedesFilter(currentPage, itemsPerPage);
            console.log(res)
    ...
    const handleCreate = async (data: ISede) => {
        try {
            await API.createSede(data);
            fetchSedes();
    ...
    const handleUpdate = async (data: ISede) => {
        try {
            if (!selectedSede?.id) return;
            await API.updateSede(selectedSede.id, data);
            fetchSedes();
    ...
    const handleModalSubmit = (data: ISede) => {
        if (selectedSede) {
            handleUpdate(data);
        } else {
            handleCreate(data);
    ...
    return (
        <div className="space-y-6 p-4">
            <h2 className="text-xl font-semibold mb-2">Registro de sedes</h2>
            <br />
            <Button
                            ...
                            filtered.map((record, idx) => (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-2 flex gap-2 justify-end">
                                        <TooltipWrapper content="Editar sede">
                                            <Button
                                                variant="outline"
                                                size="icon"
                                                onClick={() => {
                                        ...
                                        <TooltipWrapper content="Eliminar sede">
                                            <Button
                                                variant="destructive"
                                                size="icon"
